                    ...
                    int scanlines)
{
    ...
    // Stretch screen, losing original proportions.
    else
    {
        dst_width = scn_width;
    ...
    // If we're not stretching the screen, center the image.
    ...
    // We leave the element array buffer binded so it's binded when we arrive to draw_frame().
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, buffers[2]); SHOW_ERROR
